...
set设置key和value对的值，赋值到模板,例如$this->set('title', $title);

display设置模板，默认模板名称同action名称,例如$this->display();或者自己指定模板名称$this->display('edit');
...
<h5>Config类</h5>
该类主要用来方便读/写/删/检测配置的，主要包括类set/get/rm/exist方法，详细请查看kernel/Config.class.php的代码

***********************************************************************************************************************************************************************************

<h5>Lang类</h5>
该类主要用来方便读/写/删/检测语言包的，主要包括类set/get/rm/exist方法，详细请查看kernel/Lang.class.php的代码
...
初始化
在任意action类的方法中，调用$mysql = Mysql::getInstance();
...
<h5>Cahe类</h5>
初始化
在任意action类的方法中，调用$cache = Cache::getInstance(); 如果需要指定缓存类型可以这样操作$cache = Cache::getInstance('f'); //使用文件缓存模式
...
<h5>Session类</h5>
用于Session操作的相关的方法的封装，详见kernel/Session.class.php
